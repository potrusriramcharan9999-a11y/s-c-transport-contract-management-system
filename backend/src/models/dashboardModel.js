...
async function summary() {
  const result = await query(
    `SELECT
      ...
      (SELECT COUNT(*)::int FROM contracts WHERE status = 'PENDING_RENEWAL') AS pending_renewals,
      (SELECT COUNT(*)::int FROM payments WHERE payment_status = 'OVERDUE') AS overdue_payments,
      (SELECT COUNT(*)::int FROM alerts WHERE alert_type = 'RENEWAL' AND is_sent = FALSE AND alert_date >= CURRENT_DATE) AS upcoming_alerts,
      COALESCE(
